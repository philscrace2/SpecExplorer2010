...
// Type: Microsoft.VisualStudio.Shell.ProvideEditorExtensionAttribute
    ...
    public override void Register(RegistrationAttribute.RegistrationContext context)
    {
      ...
      using (RegistrationAttribute.Key key = context.CreateKey(this.RegKeyName))
      {
        if (!string.IsNullOrEmpty(this.DefaultName))
          key.SetValue((string) null, (object) this.DefaultName);
        if (this.resId != 0)
      ...
      using (RegistrationAttribute.Key key = context.CreateKey(this.EditorFactoryNotifyKey))
        key.SetValue("EditorFactoryNotify", (object) this.Factory.ToString("B"));
    }
