...
public bool TryFindLocation(MemberInfo member, out TextLocation location)
{
    try
    {
        ...
        if (member.Kind == MemberKind.Field)
        {
            ...
            {
                if (allMember.Kind == vsCMElement.vsCMElementVariable && allMember.Name == member.Name)
                {
        ...
        if (member.Kind == MemberKind.Method)
        {
            ...
            foreach (CodeElement allMember in codeType.GetAllMembers())
            {
                if (allMember.Kind == vsCMElement.vsCMElementFunction)
                {
                    ...
                    if (allMember.Name == member.Name && codeFunction != null)
                    {
        ...
        foreach (CodeElement allMember in codeType.GetAllMembers())
        {
            if (allMember.Kind == vsCMElement.vsCMElementProperty && allMember.Name == member.Name)
            {
                location = this.MakeLocation(allMember);
                return true;
            }
    ...
    {
        this.DiagMessage(DiagnosisKind.Warning, string.Format("A Spec Explorer/Visual Studio integration error occurred: \r\n{0}.\r\n Please report this error to support team.", (object)ex.ToString()), (object)null);
        location = new TextLocation();
        return false;
    }
}
